...
low-level APIs, such as OpenCL~\cite{stoneopencl} and CUDA~\cite{cudaref}, which increases the cost of
...
\item \pencil should simplify static code analysis for the optimizing compiler.  For example, the
...
to enhance static-analysis and enable mapping to GPU are needed.
...
\subsubsection{\pencil Types (Scalars, Structures, Arrays and Pointers)}
Unlike \C, unions and bitfields are not supported in \pencil.
Only the following \C types are allowed.
...
\subsubsection{Scalar Type Conversion}
Scalar type conversion in \pencil follows the same rules as in \C.
...
The \lstinline!const! and the \lstinline!restrict! \C type qualifiers are
supported in \pencil.
...
not support recursion.  Following the \C standard, function overloading
is also not allowed.
\pencil functions can be called from a \C program.
...
\subsection*{Keywords and Type Qualifiers for Array Function Arguments}
...
\item[Description] This qualifier has the same syntax and semantics
  as in C99.
  All array arguments of a \pencil function
  must be declared using the \lstinline!restrict! type qualifier.
...
\subsection{\lstinline!pencil independent! Directive\label{sec:independent}}
...
\end{description}
%--------------------------------------------------------------------------------
\subsection{\lstinline!pencil ivdep! Directive\label{sec:ivdep}}
...
\begin{itemize}
 \item \pencil code must include
   \texttt{pencil.h}~\footnote{https://github.com/carpproject/};
...
\pencil code compiled with C99 Compilers must include \texttt{pencil.h}.
