...
\item For compatibility reasons, a standard C11~\ref{c11} compiler that supports
  ...
  (we need a C11 compiler rather than a C99 compiler because the memory model defined
  in \pencil is based on the memory model of C11 and relies on the use of atomic
  functions that are a part of C11).
...
% - calls to other summaries are allowed.
  ...
  marking and tracking may be needed to attach the analyzed properties
...
\subsubsection{\texttt{\_\_pencil\_npr\_mem\_tag} Builtin}
...
% They are not equivalent, but typed (i.e. max(1,256) != bmax(1,256)).
\begin{itemize}
% MK: "max" is also a prepocessor symbol that ppcg emits into every output file and therefore directly clashes with this specification. (That defintion might match the intended effect in most cases, but not always, like if the arguments are not ints or have side-effects)
% MK: pencil_headers_generator emits "imax" for int arguments.
 \item \emph{max}: used for int arguments;
 \item \emph{smax}: used for short arguments;
 \item \emph{bmax}: used for char arguments;
% MK: pencil_headers_generator emits "fmaxd" for double arguments and omits the the "f" prefix for functions that have no integer variant.
 \item \emph{fmax}: used for double arguments;
 \item \emph{lmax}: used for long arguments;
% MK: This clashes for "fmod" - C99's math.h already has a declaration of "fmodf".
...
\end{itemize}
% MK: It is unclear whether float-only functions (sin, cos, log, ...) should get "f"-prefixes as well.
