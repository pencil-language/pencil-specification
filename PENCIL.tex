...
\begin{lstlisting}[language=pencil]
  int j = i++; //Valid in C99, invalid in \pencil.
  i++; //Valid in C99 and in \pencil.
\end{lstlisting}
...
void baz()
{
  /* Note: this is a piece of C code that calls a PENCIL function,
  it is not PENCIL code.  */
...
To map \pencil code to OpenCL and to take benefit of all the features
...
compiler (gcc, icc, \dots) as if it was C99 code (useful mainly for
debugging).
...
that two nodes, on the frontier, have an edge to the same neighbor \emph{m}.  In that
